...
const StaffScan = () => {
  const { markStaffAttendance, staff } = useGym();
      ...
      if (decodedText === 'TALEEM_GYM' || decodedText.includes('taleem')) {
        ...
        if (html5QrCode.isScanning) {
          html5QrCode.stop().then(() => {
             markStaffAttendance(staffMember).then(response => {
              setResult({
  ...
  }, [scanning, markStaffAttendance, staffMember]);

  return (
    <div className="animate-fade-in" style={{ paddingBottom: '5rem' }}>
      <div style={{ marginBottom: '3rem' }}>
        <h1 style={{ fontSize: '2.5rem', letterSpacing: '0.05em', fontWeight: '900' }}>STAFF QR</h1>
        <p style={{ color: 'var(--text-dim)', fontSize: '0.75rem', fontWeight: '800', letterSpacing: '0.1em' }}>SCAN GYM QR TO MARK ATTENDANCE</p>
      </div>
            ...
            {result ? (
              <div style={{ padding: '1rem' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                  ...
                  <button className="btn btn-primary" style={{ flex: 1, padding: '1.25rem' }} onClick={() => navigate('/staff')}>DASHBOARD</button>
                </div>
              </div>
            ) : (
              <div style={{ padding: '1rem' }}>
                ...
                {error?.includes('DENIED') || error?.includes('Permission denied') ? (
                  ...
                  <p style={{ marginBottom: '3rem', color: 'var(--text-dim)', fontWeight: '600', lineHeight: '1.6' }}>
                    {error || 'Grant camera permission to scan the gym entrance QR code to mark attendance.'}
                  </p>
                )}
                
                <button className="btn btn-primary" style={{ width: '100%', padding: '1.25rem' }} onClick={requestPermission}>
                  START SCANNER
                </button>
              </div>
            )}
